...
func getStatuses(statusQuery string) []release.Status_Code {
	if statusQuery == "" {
		...
		for _, s := range strings.Split(statusQuery, ",") {
			...
			case "pending":
				statuses = append(statuses, release.Status_PENDING_INSTALL)
			default:
...
// RollbackRelease rolls back to specific revision
...
type TillerClient interface {
	...
	TestRelease(name, namespace string) (*TestStatus, error)
