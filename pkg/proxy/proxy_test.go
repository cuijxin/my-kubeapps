...
func TestListNamespacedRelase(t *testing.T) {
	app1 := AppOverview{"foo", "1.0.0", "my_ns", "icon.png", "DEPLOYED", "wordpress", chart.Metadata{
		Version: "1.0.0",
		Icon:    "icon.png",
		Name:    "wordpress",
...
func TestListOldRelease(t *testing.T) {
	...
	if !reflect.DeepEqual([]AppOverview{appUpgraded}, releases) {
		t.Errorf("Unexpected list of reelases %v", releases)
	}
}
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			if test.shouldFail {
				...
				if strings.HasPrefix(manifest, "\n") {
					t.Errorf("The manifest should not contain new lines at the beginning")
				}
			}
		})
	}
