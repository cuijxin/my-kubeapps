...
import (
	...
	apprepoclientset "github.com/cuijxin/my-kubeapps/cmd/apprepository-controller/pkg/client/clientset/versioned"
...
// kubeHandleer handles http requests for operating on app repositories and k8s resources
...
type userHandler struct {
	...
	// clientset using the pod servcieaccount for the default cluster
	svcClientset combinedClientsetInterface
...
func (a *kubeHandler) AsUser(token, cluster string) (handler, error) {
	...
	// if the users own credential does not suffice. If a service token is not configured
...
// NewHandler returns a handler configured with a seervice account client set and a config
// with a blank token to be copied when creting user client sets with specific tokens.
...
// clientForConfig returns a clientset using the provided config.
...
// ListAppRepositories list AppRepositories in a namespace, bypass RBAC if the request namespace is the global one
